        ...
        For intCounter = 0 to colArgs.Count - 1
            ...
            Select Case CheckFile(strFilePath)
                ...
                Case CHECKFILE_FileDoesNotExist
                    WScript.Echo """" & strFilePath & """", vbCritical, "File " & intCounter & " does not exist"
                    WScript.Quit

                Case CHECKFILE_NotMSOFile
                    WScript.Echo """" & strFilePath & """", vbCritical, "File " & intCounter & " is not a valid file type"
                    WScript.Quit
            End Select
        Next
    Else
        'If there's not even one argument/file to process then exit
        WScript.Echo "Please pass a file to this script", 48,"No File Provided"
        WScript.Quit
    End If
